...
-- Muestra `lines` en un split de solo lectura con q/- para cerrar (igual que textsymbol.lua).
...
-- NOTA: se deja en jobstart (async). Si `transaction read` se colgara vía jobstart
-- (como messageclass/osql), habría que cambiarlo a vim.fn.system(); no está confirmado.
...
-- Una TCODE válida: >=3 chars de [%w_/].
...
function M.setup()
  ...
  -- NOTA: `:SapTransactionView`/`WhereUsed` dependen de `sapcli transaction read/whereused`,
  -- que en algunos tipos de transacción (p.ej. IAM/Fiori como ZPFJCG) devuelven 404. Para
  -- VER una transacción de forma fiable, ejecútala con :SapRunTransaction (WebGUI/SE93). Sin
  -- atajo para view (evita ofrecer algo que puede fallar); los comandos quedan disponibles.
end
