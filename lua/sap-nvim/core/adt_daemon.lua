...
local function script_path()
  ...
  if rt and rt[1] then return rt[1] end
...
local function on_stdout(_, data)
  ...
  -- Separar por "\n"; la ultima parte puede quedar incompleta en el buffer.
  while true do
    ...
    if line ~= "" then
      ...
      if ok and type(resp) == "table" and resp.id ~= nil then
        ...
        if cb then
          ...
          -- El llamador espera SOLO el body (o nil para fallo/HTTP >=400).
          local body = nil
...
function M.request_async(opts, cb)
  if not M.available() then cb(nil); return end
  local job = M.ensure()
  if not job then cb(nil); return end
