...
local function code_only(line)
  local l = line or ""
  if l:match("^%s*%*") then return "" end -- comentario de línea completa
  local q = l:find('"')                    -- comentario inline (aprox)
  if q then l = l:sub(1, q - 1) end
  return l
end
...
local function stmt_first_token(lines, row)
  ...
  for i = row - 1, limit, -1 do
    ...
    if c:match("%.%s*$") then start = i + 1; break end
  end
  for i = start, row do
    local tok = code_only(lines[i]):match("^%s*([%a_][%w_%-]*)")
    if tok then return tok:lower() end
  end
  return nil
end
...
local function detect_context(lines, row)
  ...
  for i = row - 1, limit, -1 do
    ...
    if c:match("^%s*endmethod%f[^%w]") then return "impl_between" end
    if c:match("^%s*method%f[^%w_]") then return "method_body" end
    if c:match("endclass%f[^%w]") then return "global" end
    if c:match("class%f[^%w_].-implementation") then return "impl_between" end
    if c:match("class%f[^%w_].-definition") then return "class_def" end
  end
  return "global"
end
...
local CONTEXT_ITEMS = {
  ...
  method_body = {
    ...
    { label = "READ TABLE", insert = "READ TABLE ${1:itab} INTO ${2:wa} WITH KEY ${3:k} = ${4:v}.", snippet = true },
  },
  global = {},
}
...
function source:get_completions(ctx, callback)
  ...
  -- Expandir variables dinámicas con el contexto del buffer actual.
  local ok_tv, tctx = pcall(function()
    return require("sap-nvim.core.template_vars").context(bufnr)
  end)
  if ok_tv then items = expand_dynamic(items, tctx) end
